...
int main(int argc, char *argv[])
{
    ...
    //dt_index_type i=0;
    //for(i=0; i<n_deformed_source; i++) printf("%s\n",src_deformed[i]);
    //return 0;
    ...
    if (argc > 3)
    {
	    ...
	    /*
	    char filename[50];
	    strcpy(filename, "pose_");   //NOTICE 20200525
	    ...
	    //printf("%s\n", filename);	
	    strcpy(src_deformed[i_source], filename);*/

            /* read deformed source model */
            printf("\nloading source deformed [%d] meshes (%s)...\n", i_source, src_deformed[i_source]);
